...
@given(parsers.parse("A first number {number}"), converters={"number": int}, target_fixture="context")
def given_a_first_number(number: int) -> None:
    return {"a": number}


@given(parsers.parse("A second number {number}"), converters={"number": int})
...
@then(parsers.parse("I get the sum {sum}"), converters={"sum": int})
